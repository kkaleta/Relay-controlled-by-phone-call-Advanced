...
int GSM::begin(long baud_rate)
{
     ...
     for (cont=0; cont<3; cont++) {
          if (AT_RESP_ERR_NO_RESP == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)&&!turnedON) {		//check power
			   ...
			   // Serial.println(F("simm800l restart GSM.cpp"));
...
// Serial.println(F("simm800l restart GSM.cpp l=287"));         
		 digitalWrite(GSM_ON, LOW);
          delay(1200);
          digitalWrite(GSM_ON, HIGH);
          delay(10000);
...
void GSM::InitParam(byte group)
{
     ...
     case PARAM_SET_0:
		  ...
		  //DOPISALEM
		  SendATCmdWaitResp(F("AT+CENG=0,0"), 500, 50, str_ok, 5);
     ...
     case PARAM_SET_1:
          ...
          SetCommLineStatus(CLS_ATCMD);
		  //USTAW PIN
		  SendATCmdWaitResp(F("AT+CPIN=2580"), 500, 50, "READY", 5);
		  //CREG DLA CID
		  SendATCmdWaitResp(F("AT+CREG=2"), 500, 50, "OK", 5);
	     //Get Local Timestamp BEZ TEGO NIE BANGLA DATA I GODZINA
		   SendATCmdWaitResp(F("AT+CLTS=1"), 500, 50, "READY", 5);
		  
          // Request calling line identification
          SendATCmdWaitResp(F("AT+CLIP=1"), 500, 50, str_ok, 5);
          // Mobile Equipment Error Code
          SendATCmdWaitResp(F("AT+CMEE=1"), 500, 50, str_ok, 5);
		  ...
		  // SendATCmdWaitResp(F("AT+CSCS=\"GSM\""), 500, 50, str_ok, 5);
		  	// Serial.println("wpisuje AT+CMGF=1 w GSM.h");
                   ...
                   char const *expected_resp_string)
{
     ...
     if (status == RX_FINISHED) {
          ...
          if(IsStringReceived(expected_resp_string)) {
			  // Serial.println("oczekiwalem");
			  // Serial.println(expected_resp_string);
               // Serial.println("i odebralem!");
...
char GSM::SendATCmdWaitResp(char const *AT_cmd_string,
                            ...
                            byte no_of_attempts)
{
	// Serial.println("SendATCmdWaitResp GSM.CPP LINIA 427...");
	// Serial.println(AT_cmd_string);
     ...
     for (i = 0; i < no_of_attempts; i++) {
          ...
          if (status == RX_FINISHED) {
			  // Serial.println(F("RX_FINISHED"));
               // something was received but what was received?
               // ---------------------------------------------
               if(IsStringReceived(response_string)) {
				   // Serial.println(F("AT_RESP_OK"));
                    ret_val = AT_RESP_OK;
                    break;  // response is OK => finish
               } else {
				   ret_val = AT_RESP_ERR_DIF_RESP;
				   // Serial.println(F("AT_RESP_ERR_DIF_RESP"));
				   // Serial.println((char *)(comm_buf));
               ...
               ret_val = AT_RESP_ERR_NO_RESP;
			    // Serial.println(F("AT_RESP_ERR_NO_RESP"));
...
char GSM::SendATCmdWaitResp(char const *AT_cmd_string,
                            uint16_t start_comm_tmout, uint16_t max_interchar_tmout,
                            char const *response_string,
                            byte no_of_attempts,char *reply)
{
     ...
     for (i = 0; i < no_of_attempts; i++) {
          ...
          if (status == RX_FINISHED) {
			  gsm.SetCommLineStatus(CLS_FREE);
               ...
               // if(IsStringReceived(response_string)) {
                    ret_val = AT_RESP_OK;
					strcpy(reply, (char *)(comm_buf));
					
					 // Serial.println("something was received, len:");
					 // Serial.println(strlen(reply));
					 // Serial.print(reply);
					 // Serial.print(reply);
                    break;  // response is OK => finish
               // } else {ret_val = AT_RESP_ERR_DIF_RESP;
			   // Serial.println("different response!");
			   // Serial.println((char *)comm_buf);
			   // }
          } else {
...
void GSM::RTCtime(int *day,int *month, int *year,int *hour,int *minute, int *second){
