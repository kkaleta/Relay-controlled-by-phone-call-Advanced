...
{
	// Serial.println("::readCellData");
     if (getStatus()==IDLE)
          return 0;

	 //SPRAWDZ TO
	 //W TYM MIJSCU MUSI BYC JUZ WPISANY SIMPIN
	 //AT+CREG=2
	 //AT+CREG?
     SimpleWriteln(F("AT+CREG=2"));

	 /*
	 +CREG: 2,1,"CE4B","0EF7"
     OK
 	 */
	 // Serial.println("SIM900.CPP #163");
	 delay(100);
     SimpleWriteln(F("AT+CREG?"));

     if(gsm.WaitResp(5000, 50, "+CREG")!=RX_FINISHED_STR_NOT_RECV)
          return 0;
// Serial.println(gsm.WaitResp(5000, 50, "+CREG"));
	// raw=(char)_cell);
     ...
     gsm.WaitResp(5000, 50, "OK"); //chyba ok bez+?
     return 1;
}
/////////////////

char SIMCOM900::readCellTimeDate(char *datetime )
{
	char ret_val=-1;
	...
	char *p_char;
    char *p_char1;
	 SimpleWriteln(F("AT+CCLK?"));
	   WaitResp(5000, 500);
	    if(gsm.IsStringReceived("+CCLK"))
		{
			 p_char = strchr((char *)(gsm.comm_buf),'\"');
			 p_char1 = p_char+1;
			strncpy(datetime, (char *)(p_char1),17);
...
int SIMCOM900::setPIN(char *pin)
{
	Serial.println("wpisuje pin ####");
     ...
     gsm.SimpleWrite(F("AT+CPIN="));
     gsm.SimpleWriteln(pin);
     //Expect str_ok.

     if(gsm.WaitResp(5000, 50, "READY")!=RX_FINISHED_STR_NOT_RECV){
...
//AT#CCID AT command returns teh identificaiton is the SIM card..
{
...
char GSM::GetPhoneNumber(byte position, char *phone_number)
{
     ...
     case RX_FINISHED_STR_RECV:
          ...
          if (p_char != NULL) {
			   ...
			   // Serial.print("GSM::GetPhoneNumber=");Serial.println(phone_number);
...
**********************************************************/
char GSM::WritePhoneNumber(byte position, char *phone_number, char *name_str)
{
     char ret_val = -1;
	 ...
	 if(strlen(phone_number)<10){ //brak prefixu kraju, dodaje
		 char combined[20] = {0}; 
		 char plus[] = "48";
		 strcat(combined, plus);
		 strcat(combined, phone_number);
		 phone_number=combined;
	 }

	 _cell.print(F("AT+CPBW="));
	 //jezeli deklaruje pozcyje (np dla Administratora to uzyj, inaczej pomin index zeby dodawac jako kolejne iteracje)
	 if (!position == 0)
     _cell.print((int)position);
     _cell.print(F(",\""));
     _cell.print(phone_number);
	 _cell.print(F("\",145,\""));
	 //cut long names to 14 characters (SIM card limitaions)
	 if(strlen(name_str)>14){
		 char cutted[15];
		 strncpy(cutted, (char *)(name_str),14);
		 cutted[14] = '\0'; 
	 _cell.print(cutted	);
	 }else
	_cell.print(name_str);	 
     _cell.print(F("\"\r"));
...
**********************************************************/
char GSM::DelPhoneNumber(char position)
{
	// Serial.print("char GSM::DelPhoneNumber @ ");
...
char GSM::getBattInf(char *st,char *str_perc, char *str_vol)
{
     char ret_val=0;
     char *p_char;
     char *p_char1;
    ...
    gsm.WaitResp(5000, 100, "OK");
     if(gsm.IsStringReceived("+CBC"))
          ret_val=1;
	 p_char = strchr((char *)(gsm.comm_buf),'+CBC:');
	 p_char1=p_char+2;
	 p_char1 = p_char+2; // we are on the first phone number character
     p_char = strchr((char *)(p_char1),',');
        if (p_char != NULL) {*p_char = 0; }
            strcpy(st, (char *)(p_char1));
	p_char++;	 
	p_char1 = strchr((char *)(p_char), ',');
	 if (p_char1 != NULL) {*p_char1 = 0;}
     strcpy(str_perc, (char *)(p_char));
	 strcat(str_perc, "%");
	 p_char1++;
     p_char = strchr((char *)(p_char1), '" "');
     if (p_char != NULL) {
	 ...
	 strncpy(str_vol, (char *)(p_char1),4);
	 float f = (atof(str_vol));
	 	 char buffor[5]; 
	 dtostrf(f/1000,2,2,buffor);
	  strcpy(str_vol, buffor);
	  strcat(str_vol, "V");
