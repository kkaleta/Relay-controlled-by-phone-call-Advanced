...
{
	// Serial.println(F("char SMSGSM::SendSMS(char *number_str, char *message_str)"));
...
{
	// Serial.println(F("SMSGSM::SendSMS(byte sim_phonebook_position, char *message_str)"));
	// Serial.println(sim_phonebook_position);
	// Serial.println(message_str);
...
char SMSGSM::IsPhonePresent(char *is_phone_number){
	 ...
	 gsm.SimpleWriteln(F("AT+CPBR=1,250"));
	 gsm.RxInit(5000, 500);
	do {
          if (gsm.IsStringReceived("OK")) {
			   ...
			   // Serial.println("gsm.IsStringReceived(NOK)");
		  }
     ...
     case RX_FINISHED:
          ...
          // ---------------------------------------------
          if(gsm.IsStringReceived(is_phone_number)) {
			 
			   
			 p_char = strchr((char *)gsm.comm_buf,is_phone_number);
	 ...
	 // Serial.print("dlugosc bufora=");
	 // Serial.println(strlen(gsm.comm_buf));
...
char SMSGSM::IsSMSPresent(byte required_status)
{
     ...
     gsm.SetCommLineStatus(CLS_ATCMD);
     ret_val = 0; // still not present
	// Serial.print("\nISSMSPORESENT status");
	// Serial.println(required_status);
     switch (required_status) {
     case SMS_UNREAD:
          gsm.SimpleWriteln(F("AT+CMGL=\"REC UNREAD\""));
		  // Serial.println(" SMS_UNREAD");
          break;
     case SMS_READ:
     ...
     case RX_FINISHED:
          ...
          if(gsm.IsStringReceived("+CMGL:")) {
               ...
               // other response like OK or ERROR
			    // Serial.println("tu sie wyjebalem");
               ret_val = 0;
          }
...
{
	// Serial.println(F("::GetSMS"));
     char ret_val = -1;
     char *p_char;
     char *p_char1;
     byte len;

	 // Serial.print("::GetSMS position =");
	 // Serial.println(position);
     if (position == 0) return (-3);
     if (CLS_FREE != gsm.GetCommLineStatus()) 
	 {	

		// Serial.print(F("...CLS_FREE != gsm.GetCommLineStatus()")); 
		return (ret_val);
     }
	 gsm.SetCommLineStatus(CLS_ATCMD);
	...
	// Serial.print("...gsm.WaitResp(5000, 100, \"+CMGR\")=");
	// Serial.println(gsm.WaitResp(5000, 100, "+CMGR"));
     // 5000 msec. for initial comm tmout
     // 100 msec. for inter character tmout
     switch (gsm.WaitResp(10000, 100, "+CMGR")) {
     case RX_TMOUT_ERR:
     ...
     case RX_FINISHED_STR_NOT_RECV:
	  //Serial.println(F("..RX_FINISHED_STR_NOT_RECV"));
                              ...
                              byte first_authorized_pos, byte last_authorized_pos)
{
	//Serial.println(F("::GetAuthorizedSMS"));
	 ...
	 //Serial.print(F("ret_val na koncu="));Serial.print((int)ret_val);
     return (ret_val);
}
...
char SMSGSM::DeleteSMS(byte position)
{
	 ...
	 // Serial.print("try delete  sms at position ");
